...
def test_scut_bootstrap_exposes_site_specific_interest_profiles():
    ...
    assert outdoor["metadata"]["interest_calibration_stage"] == "M31C_SCUT"
    ...
    assert payload["default_user_id"] == "user_scut_001"
    assert {"user_scut_001", "user_scut_002", "user_scut_003"} <= user_ids
    assert {"图书馆", "工科教学科研区", "西湖", "校史", "东区饭堂", "校园生活"} <= interest_values


def test_scut_interest_recommendation_prefers_local_landmarks_and_canteens():
    service = DemoUIService("SCUT")

    study_response = service.scenic_search(
        {
            "user_id": "user_scut_001",
            "category": "education",
            "sort_field": "interest",
    ...
    assert study_response["metadata"]["user_interest_context"]["user_id"] == "user_scut_001"
    assert landmark_response["metadata"]["user_interest_context"]["user_id"] == "user_scut_002"
    assert campus_life_response["metadata"]["user_interest_context"]["user_id"] == "user_scut_003"
    ...
    assert landmark_top[0] == "west_lake"
    assert {"west_lake", "history_museum"} <= set(landmark_top[:3])
    assert campus_life_top[0] == "canteen"
    assert "canteen_east" in campus_life_top[:3]
    ...
    assert any(token in study_response["results"][0]["interest_reason"] for token in ("图书馆", "自习", "工科教学科研区"))
    assert any(token in landmark_response["results"][0]["interest_reason"] for token in ("西湖", "校史", "摄影"))
    assert any(token in campus_life_response["results"][0]["interest_reason"] for token in ("东区饭堂", "食堂", "校园生活"))
