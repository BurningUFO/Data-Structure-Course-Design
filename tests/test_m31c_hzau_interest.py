...
def test_hzau_bootstrap_exposes_site_specific_interest_profiles():
    ...
    assert outdoor["metadata"]["interest_calibration_stage"] == "M31C_HZAU"
    assert highlights["study"]["highlight_node_ids"] == ["library", "agronomy_building", "lab_building"]
    ...
    assert {"user_hzau_001", "user_hzau_002", "user_hzau_003"} <= user_ids
    assert {"图书馆", "农业科技", "狮子山", "校史", "荟园", "校园生活"} <= interest_values


def test_hzau_interest_recommendation_prefers_local_landmarks_and_canteens():
    service = DemoUIService("HZAU")
    ...
    assert study_response["metadata"]["user_interest_context"]["user_id"] == "user_hzau_001"
    assert landmark_response["metadata"]["user_interest_context"]["user_id"] == "user_hzau_002"
    assert campus_life_response["metadata"]["user_interest_context"]["user_id"] == "user_hzau_003"
    ...
    assert {"agronomy_building", "lab_building"} <= set(study_top[:6])
    assert landmark_top[0] == "shizishan_landmark"
    assert {"shizishan_landmark", "history_museum"} <= set(landmark_top[:3])
    assert campus_life_top[0] == "canteen_huiyuan"
    ...
    assert any(token in study_response["results"][0]["interest_reason"] for token in ("图书馆", "自习", "农业科技"))
    assert any(token in landmark_response["results"][0]["interest_reason"] for token in ("狮子山", "摄影"))
    assert any(token in campus_life_response["results"][0]["interest_reason"] for token in ("荟园", "食堂", "校园生活"))
