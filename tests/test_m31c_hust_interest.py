...
def test_hust_bootstrap_exposes_site_specific_interest_profiles():
    ...
    assert outdoor["metadata"]["interest_calibration_stage"] == "M31C_HUST"
    ...
    assert payload["default_user_id"] == "user_hust_001"
    assert {"user_hust_001", "user_hust_002", "user_hust_003"} <= user_ids
    assert {"图书馆", "东九楼", "喻家山", "校史", "韵苑", "校园生活"} <= interest_values


def test_hust_interest_recommendation_prefers_local_landmarks_and_canteens():
    service = DemoUIService("HUST")

    study_response = service.scenic_search(
        {
            "user_id": "user_hust_001",
            "category": "education",
            "sort_field": "interest",
    ...
    assert study_response["metadata"]["user_interest_context"]["user_id"] == "user_hust_001"
    assert landmark_response["metadata"]["user_interest_context"]["user_id"] == "user_hust_002"
    assert campus_life_response["metadata"]["user_interest_context"]["user_id"] == "user_hust_003"
    ...
    assert landmark_top[0] == "yujia_mountain"
    assert {"yujia_mountain", "history_museum"} <= set(landmark_top[:3])
    assert {"canteen", "canteen_yunyuan"} <= set(campus_life_top[:3])
    ...
    assert any(token in study_response["results"][0]["interest_reason"] for token in ("图书馆", "自习", "东九楼"))
    assert any(token in landmark_response["results"][0]["interest_reason"] for token in ("喻家山", "校史", "摄影"))
    assert any(token in campus_life_response["results"][0]["interest_reason"] for token in ("韵苑", "食堂", "校园生活"))
