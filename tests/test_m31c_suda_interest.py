...
def test_suda_bootstrap_exposes_site_specific_interest_profiles():
    ...
    assert outdoor["metadata"]["interest_calibration_stage"] == "M31C_SUDA"
    ...
    assert payload["default_user_id"] == "user_suda_001"
    assert {"user_suda_001", "user_suda_002", "user_suda_003"} <= user_ids
    assert {"图书馆", "苏大红楼", "红楼", "校史", "东区餐厅", "校园生活"} <= interest_values


def test_suda_interest_recommendation_prefers_local_landmarks_and_canteens():
    service = DemoUIService("SUDA")

    study_response = service.scenic_search(
        {
            "user_id": "user_suda_001",
            "category": "education",
            "sort_field": "interest",
    ...
    assert study_response["metadata"]["user_interest_context"]["user_id"] == "user_suda_001"
    assert landmark_response["metadata"]["user_interest_context"]["user_id"] == "user_suda_002"
    assert campus_life_response["metadata"]["user_interest_context"]["user_id"] == "user_suda_003"
    ...
    assert {"teaching_building", "lab_building"} <= set(study_top[:6])
    assert landmark_top[0] == "red_building_complex"
    assert {"red_building_complex", "history_museum"} <= set(landmark_top[:3])
    assert campus_life_top[0] == "canteen"
    assert "canteen_east" in campus_life_top[:6]
    ...
    assert any(token in study_response["results"][0]["interest_reason"] for token in ("图书馆", "自习", "苏大红楼"))
    assert any(token in landmark_response["results"][0]["interest_reason"] for token in ("红楼", "校史", "摄影"))
    assert any(token in campus_life_response["results"][0]["interest_reason"] for token in ("东区餐厅", "食堂", "校园生活"))
