...
def test_m31a_ynu_transport_summary_and_step_copy():
    service = DemoUIService("YNU")
    ...
    assert mixed_route["summary"]["transport_text"] == "步行 + 自行车最短时间"
    assert mixed_route["summary"]["strategy_text"] == "最短时间"
    assert mixed_route["site_id"] == "YNU"
    assert mixed_route["ui"]["route_geojson"] is not None
    ...
    assert "体育馆骑行落客接驳" in bike_route["path_steps"][-1]["description"]
    assert mixed_route["path_steps"][0]["allowed_transports"] == ["walk"]
    assert "云南大学呈贡校区南门步行短接" in mixed_route["path_steps"][0]["description"]
    assert any(
        "云南大学呈贡校区南区步骑共享主路示范段" in step["description"]
        for step in mixed_route["path_steps"]
    )


def test_m31a_hzau_transport_summary_and_step_copy():
    service = DemoUIService("HZAU")
    ...
    assert mixed_route["summary"]["transport_text"] == "步行 + 自行车最短时间"
    assert mixed_route["summary"]["strategy_text"] == "最短时间"
    assert mixed_route["site_id"] == "HZAU"
    assert mixed_route["ui"]["route_geojson"] is not None
    ...
    assert "体育馆骑行落客接驳" in bike_route["path_steps"][-1]["description"]
    assert mixed_route["path_steps"][0]["allowed_transports"] == ["walk"]
    assert "华中农业大学狮子山校区南门步行短接" in mixed_route["path_steps"][0]["description"]
    assert any(
        "华中农业大学狮子山校区步骑共享主路示范段" in step["description"]
        for step in mixed_route["path_steps"]
    )
...
def test_demo_m31b_whu_bootstrap_nearby_profiles():
    service = DemoUIService("WHU")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]

    assert payload["site"]["id"] == "WHU"
    assert [item["value"] for item in payload["controls"]["nearby_radius_options"]] == [200, 300, 500, 600, 800]
    assert profiles["library"]["default_radius_m"] == 200.0
    assert profiles["library"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "桂园学生宿舍"
    assert profiles["dormitory_1"]["default_category"] == "catering"
    assert profiles["main_arch"]["default_category"] == "service"
    assert profiles["old_library"]["default_radius_m"] == 600.0
    print("test_demo_m31b_whu_bootstrap_nearby_profiles passed.")


def test_demo_m31b_whu_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("WHU")

    dorm_catering = service.place_search(
        {
            "keyword": "",
            "category": "catering",
            "center_node_id": "dormitory_1",
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "桂园学生宿舍"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_WHU"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离桂园学生宿舍 ")

    old_library_catering = service.place_search(
        {
            "keyword": "",
            "category": "catering",
            "center_node_id": "old_library",
            "radius_m": 600,
            "limit": 10,
        }
    )
    assert_nearby_place_response(
        old_library_catering,
        center_node_id="old_library",
        radius_m=600,
        category="catering",
    )
    assert old_library_catering["metadata"]["nearby"]["center_name"] == "武汉大学老图书馆"
    assert old_library_catering["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 600.0
    assert old_library_catering["results"][0]["route_target_node_id"] == "canteen"
...
def test_demo_m31b_xmu_bootstrap_nearby_profiles():
    ...
    assert profiles["library"]["default_category"] == "service"
    assert profiles["dormitory_1"]["default_category"] == "catering"
    assert profiles["nanguang_dormitory"]["default_category"] == "shopping"
    assert profiles["gate_south"]["center_name"] == "厦门大学白城校门"
    assert profiles["sports_ground"]["default_category"] == "sports"
    print("test_demo_m31b_xmu_bootstrap_nearby_profiles passed.")


def test_demo_m31b_xmu_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("XMU")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "厦门大学芙蓉学生公寓"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_XMU"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离厦门大学芙蓉学生公寓 ")

    gate_south_restroom = service.place_search(
        {
            "keyword": "",
            "category": "restroom",
    ...
    assert gate_south_restroom["metadata"]["nearby"]["center_name"] == "厦门大学白城校门"
    assert gate_south_restroom["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 200.0
    assert gate_south_restroom["results"][0]["route_target_node_id"] == "restroom_south"
    print("test_demo_m31b_xmu_nearby_queries_use_calibrated_center_name_and_scope passed.")


def test_demo_m31b_zju_bootstrap_nearby_profiles():
    service = DemoUIService("ZJU")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]

    assert payload["site"]["id"] == "ZJU"
    assert [item["value"] for item in payload["controls"]["nearby_radius_options"]] == [200, 300, 400, 600, 800]
    assert profiles["library"]["default_radius_m"] == 200.0
    assert profiles["library"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "浙江大学丹青学园"
    assert profiles["dormitory_1"]["default_category"] == "catering"
    assert profiles["dormitory_2"]["default_radius_m"] == 200.0
    assert profiles["qiushi_square"]["default_category"] == "landmark"
    assert profiles["gate_south"]["default_radius_m"] == 600.0
    assert profiles["sports_ground"]["default_category"] == "sports"
    print("test_demo_m31b_zju_bootstrap_nearby_profiles passed.")


def test_demo_m31b_zju_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("ZJU")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "浙江大学丹青学园"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_ZJU"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen_yinquan"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离浙江大学丹青学园 ")

    gate_south_education = service.place_search(
        {
            "keyword": "",
            "category": "education",
            "center_node_id": "gate_south",
...
def test_demo_m31b_nju_bootstrap_nearby_profiles():
    service = DemoUIService("NJU")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]
    ...
    assert profiles["library"]["default_radius_m"] == 200.0
    assert profiles["library"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "南京大学仙林学生宿舍一组团"
    assert profiles["dormitory_1"]["default_category"] == "shopping"
    assert profiles["gate_south"]["default_radius_m"] == 800.0
    assert profiles["service_center"]["default_category"] == "service"
    assert profiles["sports_center"]["default_category"] == "sports"
    print("test_demo_m31b_nju_bootstrap_nearby_profiles passed.")


def test_demo_m31b_nju_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("NJU")
    ...
    assert dorm_shopping["metadata"]["nearby"]["center_name"] == "南京大学仙林学生宿舍一组团"
    assert dorm_shopping["metadata"]["nearby"]["calibration_stage"] == "M31B_NJU"
    assert dorm_shopping["metadata"]["nearby"]["calibration_profile"]["default_category"] == "shopping"
    assert dorm_shopping["results"][0]["route_target_node_id"] == "convenience_store"
    assert dorm_shopping["results"][0]["nearby_reason"].startswith("距离南京大学仙林学生宿舍一组团 ")
    ...
    assert gate_south_education["metadata"]["nearby"]["center_name"] == "南京大学仙林校区南门"
    assert gate_south_education["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 800.0
    assert gate_south_education["results"][0]["route_target_node_id"] == "teaching_building"
    print("test_demo_m31b_nju_nearby_queries_use_calibrated_center_name_and_scope passed.")


def test_demo_m31b_fdu_bootstrap_nearby_profiles():
    service = DemoUIService("FDU")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]
    ...
    assert profiles["library"]["default_radius_m"] == 200.0
    assert profiles["library"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "复旦大学邯郸校区南区学生宿舍"
    assert profiles["dormitory_1"]["default_category"] == "shopping"
    assert profiles["dormitory_2"]["default_category"] == "catering"
    assert profiles["gate_south"]["default_category"] == "landmark"
    assert profiles["guanghua_tower"]["default_radius_m"] == 200.0
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_fdu_bootstrap_nearby_profiles passed.")


def test_demo_m31b_fdu_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("FDU")
    ...
    assert dorm_shopping["metadata"]["nearby"]["center_name"] == "复旦大学邯郸校区南区学生宿舍"
    assert dorm_shopping["metadata"]["nearby"]["calibration_stage"] == "M31B_FDU"
    assert dorm_shopping["metadata"]["nearby"]["calibration_profile"]["default_category"] == "shopping"
    assert dorm_shopping["results"][0]["route_target_node_id"] == "convenience_store"
    assert dorm_shopping["results"][0]["nearby_reason"].startswith("距离复旦大学邯郸校区南区学生宿舍 ")

    gate_south_landmark = service.place_search(
        {
            "keyword": "",
            "category": "landmark",
            "center_node_id": "gate_south",
    ...
    assert gate_south_landmark["metadata"]["nearby"]["center_name"] == "复旦大学邯郸校区正门"
    assert gate_south_landmark["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 300.0
    assert gate_south_landmark["results"][0]["route_target_node_id"] == "guanghua_tower"
    print("test_demo_m31b_fdu_nearby_queries_use_calibrated_center_name_and_scope passed.")


def test_demo_m31b_sjtu_bootstrap_nearby_profiles():
    service = DemoUIService("SJTU")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]
    ...
    assert profiles["library"]["default_radius_m"] == 200.0
    assert profiles["library"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "上海交通大学闵行校区北区学生宿舍"
    assert profiles["dormitory_1"]["default_category"] == "catering"
    assert profiles["dormitory_2"]["default_radius_m"] == 400.0
    assert profiles["gate_south"]["default_radius_m"] == 600.0
    assert profiles["service_center"]["default_category"] == "shopping"
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_sjtu_bootstrap_nearby_profiles passed.")


def test_demo_m31b_sjtu_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("SJTU")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "上海交通大学闵行校区北区学生宿舍"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_SJTU"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen_north"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离上海交通大学闵行校区北区学生宿舍 ")
    ...
    assert gate_south_landmark["metadata"]["nearby"]["center_name"] == "上海交通大学闵行校区南门"
    assert gate_south_landmark["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 600.0
    assert [item["route_target_node_id"] for item in gate_south_landmark["results"][:2]] == [
        "culture_square",
        "siyuan_lake",
    ]
    print("test_demo_m31b_sjtu_nearby_queries_use_calibrated_center_name_and_scope passed.")


def test_demo_m31b_tongji_bootstrap_nearby_profiles():
    service = DemoUIService("TONGJI")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]
    ...
    assert profiles["dormitory_1"]["center_name"] == "同济大学四平路校区西区学生宿舍"
    assert profiles["dormitory_2"]["default_radius_m"] == 400.0
    assert profiles["gate_south"]["default_radius_m"] == 500.0
    assert profiles["service_center"]["default_category"] == "shopping"
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_tongji_bootstrap_nearby_profiles passed.")


def test_demo_m31b_tongji_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("TONGJI")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "同济大学四平路校区西区学生宿舍"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_TONGJI"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen_north"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离同济大学四平路校区西区学生宿舍 ")
    ...
    assert gate_south_landmark["metadata"]["nearby"]["center_name"] == "同济大学四平路校区南门"
    assert gate_south_landmark["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 500.0
    assert [item["route_target_node_id"] for item in gate_south_landmark["results"][:2]] == [
        "auditorium",
        "sakura_avenue",
    ]
    print("test_demo_m31b_tongji_nearby_queries_use_calibrated_center_name_and_scope passed.")


def test_demo_m31b_seu_bootstrap_nearby_profiles():
    service = DemoUIService("SEU")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]
    ...
    assert profiles["library"]["default_category"] == "service"
    assert profiles["canteen"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "东南大学九龙湖校区桃园学生宿舍"
    assert profiles["dormitory_2"]["default_radius_m"] == 400.0
    assert profiles["gate_south"]["default_radius_m"] == 800.0
    assert profiles["service_center"]["default_category"] == "shopping"
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_seu_bootstrap_nearby_profiles passed.")


def test_demo_m31b_seu_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("SEU")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "东南大学九龙湖校区桃园学生宿舍"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_SEU"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离东南大学九龙湖校区桃园学生宿舍 ")
...
def test_demo_m31b_sysu_bootstrap_nearby_profiles():
    service = DemoUIService("SYSU")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]
    ...
    assert profiles["library"]["default_category"] == "service"
    assert profiles["canteen"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "中山大学南校园西区学生宿舍"
    assert profiles["dormitory_2"]["default_radius_m"] == 500.0
    assert profiles["gate_south"]["default_radius_m"] == 800.0
    assert profiles["service_center"]["default_category"] == "catering"
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_sysu_bootstrap_nearby_profiles passed.")


def test_demo_m31b_sysu_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("SYSU")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "中山大学南校园西区学生宿舍"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_SYSU"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离中山大学南校园西区学生宿舍 ")
    ...
    assert gate_south_landmark["metadata"]["nearby"]["center_name"] == "中山大学广州校区南校园南门"
    assert gate_south_landmark["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 800.0
    assert [item["route_target_node_id"] for item in gate_south_landmark["results"][:2]] == [
        "huaishi_hall",
        "sun_yat_sen_statue",
    ]
    print("test_demo_m31b_sysu_nearby_queries_use_calibrated_center_name_and_scope passed.")


def test_demo_m31b_scu_bootstrap_nearby_profiles():
    service = DemoUIService("SCU")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]
    ...
    assert profiles["library"]["default_category"] == "service"
    assert profiles["canteen"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "四川大学望江校区西区学生宿舍"
    assert profiles["dormitory_2"]["default_radius_m"] == 500.0
    assert profiles["gate_south"]["default_radius_m"] == 800.0
    assert profiles["service_center"]["default_category"] == "catering"
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_scu_bootstrap_nearby_profiles passed.")


def test_demo_m31b_scu_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("SCU")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "四川大学望江校区西区学生宿舍"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_SCU"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离四川大学望江校区西区学生宿舍 ")
    ...
    assert gate_south_landmark["metadata"]["nearby"]["center_name"] == "四川大学望江校区南门"
    assert gate_south_landmark["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 800.0
    assert [item["route_target_node_id"] for item in gate_south_landmark["results"][:2]] == [
        "bell_tower",
        "history_museum",
...
def test_demo_m31b_hnu_bootstrap_nearby_profiles():
    ...
    assert profiles["library"]["default_category"] == "service"
    assert profiles["canteen"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "湖南大学天马学生公寓"
    assert profiles["dormitory_1"]["default_category"] == "catering"
    assert profiles["dormitory_2"]["default_radius_m"] == 500.0
    assert profiles["gate_south"]["default_category"] == "education"
    assert profiles["gate_east"]["default_radius_m"] == 300.0
    assert profiles["yuelu_academy"]["default_category"] == "landmark"
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_hnu_bootstrap_nearby_profiles passed.")


def test_demo_m31b_hnu_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("HNU")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "湖南大学天马学生公寓"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_HNU"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen_tianma"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离湖南大学天马学生公寓 ")
    ...
    assert gate_east_service["metadata"]["nearby"]["center_name"] == "湖南大学岳麓山校区东门"
    ...
    print("test_demo_m31b_hnu_nearby_queries_use_calibrated_center_name_and_scope passed.")


def test_demo_m31b_sdu_bootstrap_nearby_profiles():
    service = DemoUIService("SDU")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]
    ...
    assert profiles["library"]["default_category"] == "service"
    assert profiles["canteen"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "山东大学中心校区学生公寓"
    assert profiles["dormitory_1"]["default_category"] == "catering"
    ...
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_sdu_bootstrap_nearby_profiles passed.")


def test_demo_m31b_sdu_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("SDU")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "山东大学中心校区学生公寓"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_SDU"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离山东大学中心校区学生公寓 ")
...
def test_demo_m31b_hust_bootstrap_nearby_profiles():
    service = DemoUIService("HUST")
    payload = service.get_bootstrap_payload()
    profiles = payload["controls"]["nearby_profiles"]
    ...
    assert profiles["library"]["default_category"] == "service"
    assert profiles["canteen"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "华中科技大学韵苑学生公寓"
    assert profiles["dormitory_1"]["default_category"] == "catering"
    assert profiles["dormitory_2"]["default_category"] == "education"
    assert profiles["gate_north"]["default_radius_m"] == 500.0
    assert profiles["central_square"]["default_category"] == "catering"
    assert profiles["history_museum"]["default_category"] == "service"
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_hust_bootstrap_nearby_profiles passed.")


def test_demo_m31b_hust_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("HUST")
    ...
    assert dorm_catering["metadata"]["nearby"]["center_name"] == "华中科技大学韵苑学生公寓"
    assert dorm_catering["metadata"]["nearby"]["calibration_stage"] == "M31B_HUST"
    assert dorm_catering["metadata"]["nearby"]["calibration_profile"]["default_category"] == "catering"
    assert dorm_catering["results"][0]["route_target_node_id"] == "canteen_yunyuan"
    assert dorm_catering["results"][0]["nearby_reason"].startswith("距离华中科技大学韵苑学生公寓 ")

    gate_north_sports = service.place_search(
        {
            "keyword": "",
            "category": "sports",
            "center_node_id": "gate_north",
            "radius_m": 500,
            "limit": 10,
        }
    )
    ...
    assert gate_north_sports["metadata"]["nearby"]["center_name"] == "华中科技大学主校区北门"
    assert gate_north_sports["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 500.0
    assert [item["route_target_node_id"] for item in gate_north_sports["results"][:2]] == [
        "sports_ground",
        "gymnasium",
    ]
...
def test_demo_m31b_ouc_bootstrap_nearby_profiles():
    ...
    assert payload["site"]["id"] == "OUC"
    assert [item["value"] for item in payload["controls"]["nearby_radius_options"]] == [200, 300, 400, 500, 600, 800]
    assert profiles["library"]["default_radius_m"] == 200.0
    assert profiles["library"]["default_category"] == "service"
    assert profiles["dormitory_1"]["center_name"] == "中国海洋大学崂山校区学生宿舍区"
    assert profiles["dormitory_1"]["default_category"] == "shopping"
    assert profiles["dormitory_2"]["default_radius_m"] == 600.0
    assert profiles["gate_south"]["default_radius_m"] == 400.0
    assert profiles["gate_north"]["default_category"] == "sports"
    assert profiles["central_square"]["default_category"] == "landmark"
    assert profiles["gymnasium"]["default_category"] == "sports"
    print("test_demo_m31b_ouc_bootstrap_nearby_profiles passed.")


def test_demo_m31b_ouc_nearby_queries_use_calibrated_center_name_and_scope():
    service = DemoUIService("OUC")
    ...
    assert dorm_shopping["metadata"]["nearby"]["center_name"] == "中国海洋大学崂山校区学生宿舍区"
    assert dorm_shopping["metadata"]["nearby"]["calibration_stage"] == "M31B_OUC"
    assert dorm_shopping["metadata"]["nearby"]["calibration_profile"]["default_category"] == "shopping"
    assert dorm_shopping["results"][0]["route_target_node_id"] == "convenience_store"
    assert dorm_shopping["results"][0]["nearby_reason"].startswith("距离中国海洋大学崂山校区学生宿舍区 ")
    ...
    assert gate_north_sports["metadata"]["nearby"]["center_name"] == "中国海洋大学崂山校区北门"
    assert gate_north_sports["metadata"]["nearby"]["calibration_stage"] == "M31B_OUC"
    assert gate_north_sports["metadata"]["nearby"]["calibration_profile"]["default_radius_m"] == 500.0
    assert gate_north_sports["metadata"]["nearby"]["calibration_profile"]["default_category"] == "sports"
    assert [item["route_target_node_id"] for item in gate_north_sports["results"][:2]] == [
        "sports_ground",
        "gymnasium",
    ]
    assert gate_north_sports["results"][0]["nearby_reason"].startswith("距离中国海洋大学崂山校区北门 ")
...
def run_all_tests():
    ...
    test_demo_m31b_zju_nearby_queries_use_calibrated_center_name_and_scope()
    test_demo_m31b_nju_bootstrap_nearby_profiles()
    test_demo_m31b_nju_nearby_queries_use_calibrated_center_name_and_scope()
    test_demo_m31b_fdu_bootstrap_nearby_profiles()
    test_demo_m31b_fdu_nearby_queries_use_calibrated_center_name_and_scope()
    test_demo_m31b_sjtu_bootstrap_nearby_profiles()
    test_demo_m31b_sjtu_nearby_queries_use_calibrated_center_name_and_scope()
    test_demo_m31b_tongji_bootstrap_nearby_profiles()
    test_demo_m31b_tongji_nearby_queries_use_calibrated_center_name_and_scope()
    test_demo_m31b_seu_bootstrap_nearby_profiles()
    test_demo_m31b_seu_nearby_queries_use_calibrated_center_name_and_scope()
    test_demo_m31b_sysu_bootstrap_nearby_profiles()
