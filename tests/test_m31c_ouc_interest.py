...
def test_ouc_bootstrap_exposes_site_specific_interest_profiles():
    ...
    assert outdoor["metadata"]["interest_calibration_stage"] == "M31C_OUC"
    ...
    assert payload["default_user_id"] == "user_ouc_001"
    assert {"user_ouc_001", "user_ouc_002", "user_ouc_003"} <= user_ids
    assert {"图书馆", "海洋科学", "海洋文化", "校史", "北区餐厅", "校园生活"} <= interest_values


def test_ouc_interest_recommendation_prefers_local_landmarks_and_canteens():
    service = DemoUIService("OUC")

    study_response = service.scenic_search(
        {
            "user_id": "user_ouc_001",
            "category": "education",
            "sort_field": "interest",
    ...
    assert study_response["metadata"]["user_interest_context"]["user_id"] == "user_ouc_001"
    assert landmark_response["metadata"]["user_interest_context"]["user_id"] == "user_ouc_002"
    assert campus_life_response["metadata"]["user_interest_context"]["user_id"] == "user_ouc_003"
    ...
    assert landmark_top[0] == "ocean_culture_axis"
    assert {"ocean_culture_axis", "history_museum"} <= set(landmark_top[:3])
    ...
    assert any(token in study_response["results"][0]["interest_reason"] for token in ("图书馆", "自习", "海洋科学"))
    assert any(token in landmark_response["results"][0]["interest_reason"] for token in ("海洋文化", "校史", "摄影"))
