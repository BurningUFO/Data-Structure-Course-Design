...
class TestRouting(unittest.TestCase):
    ...
    def test_m31a_hit_transport_calibration_walk_bike_and_mixed(self):
        router = Router(GraphLoader.load_site_graph("HIT"))

        walk_to_sports = router.query_routing(
            "gate_south",
            "sports_ground",
            strategy="shortest_time",
            transport_mode="walk",
        ...
        bike_to_sports = router.query_routing(
            "gate_south",
            "sports_ground",
            strategy="shortest_time",
            transport_mode="bike",
        ...
        mixed_to_sports = router.query_routing(
            "gate_south",
            "sports_ground",
            strategy="shortest_time",
            transport_mode="mixed",
        ...
        self.assertTrue(walk_to_sports["success"])
        self.assertTrue(bike_to_sports["success"])
        self.assertTrue(mixed_to_sports["success"])
        self.assertLess(bike_to_sports["total_weight"], walk_to_sports["total_weight"])
        self.assertLess(mixed_to_sports["total_weight"], bike_to_sports["total_weight"])
        self.assertEqual(
            {step["transport_mode_used"] for step in walk_to_sports["path_steps"]},
            {"walk"},
        )
        self.assertEqual(
            {step["transport_mode_used"] for step in bike_to_sports["path_steps"]},
            {"bike"},
        )
        mixed_modes = [step["transport_mode_used"] for step in mixed_to_sports["path_steps"]]
        self.assertIn("walk", mixed_modes)
        self.assertIn("bike", mixed_modes)
        self.assertEqual(mixed_modes[0], "walk")
        self.assertEqual(bike_to_sports["path_steps"][0]["edge_type"], "bike_lane")
        self.assertEqual(bike_to_sports["path_steps"][-1]["edge_type"], "bike_lane")
        self.assertEqual(mixed_to_sports["path_steps"][0]["allowed_transports"], ["walk"])
        self.assertTrue(
            any(
                "哈尔滨工业大学一校区步骑共享主路示范段" in step["description"]
                for step in mixed_to_sports["path_steps"]
            )
        )

    def test_m31a_hit_transport_keeps_indoor_segments_walk_only(self):
        router = Router(GraphLoader.load_site_graph("HIT"))
    ...
    def test_m31a_hit_outdoor_data_declares_transport_semantics(self):
        outdoor_path = Path(__file__).resolve().parents[1] / "data" / "sites" / "HIT" / "outdoor.json"
        with outdoor_path.open("r", encoding="utf-8") as f:
            outdoor = json.load(f)

        edges = outdoor["edges"]
        calibration_source = "m31a_hit_transport_calibration"
        expected_semantics = {
            "pedestrian_gate_shortcut",
            "shared_walk_bike",
            "bike_only",
            "bike_dismount_connector",
        }
        semantic_edges = [
            edge for edge in edges
        ...
        self.assertEqual(outdoor["metadata"]["transport_calibration_stage"], "M31A_HIT")
        self.assertEqual(outdoor["metadata"]["transport_modes"], ["walk", "bike", "mixed"])
        self.assertEqual(len(semantic_edges), 16)
        self.assertEqual(len(shared_edges), 8)
        self.assertEqual(len(bike_only_edges), 2)
        self.assertEqual(len(pedestrian_gate_edges), 2)
        self.assertEqual(len(bike_dismount_edges), 4)
        self.assertTrue(all(edge.get("source") == calibration_source for edge in semantic_edges))
        self.assertTrue(all(set(edge.get("allowed_transports", [])) == {"walk", "bike"} for edge in shared_edges))
        self.assertTrue(all(edge.get("vehicle_access") == "all" for edge in shared_edges))
        self.assertEqual(
            {edge.get("type") for edge in shared_edges},
            {
                "campus_learning_road",
                "campus_catering_road",
                "campus_south_road",
                "campus_sports_road",
            },
        )
        ...
        self.assertTrue(all(edge.get("type") == "campus_gate_link" for edge in pedestrian_gate_edges))
        ...
        self.assertTrue(all(edge.get("allowed_transports") == ["bike"] for edge in bike_dismount_edges))
        self.assertTrue(all(edge.get("vehicle_access") == "vehicle_only" for edge in bike_dismount_edges))
        self.assertTrue(all(edge.get("type") == "bike_lane" for edge in bike_dismount_edges))
        self.assertTrue(
            all("哈尔滨工业大学" in f"{edge.get('name', '')}{edge.get('description', '')}" for edge in bike_only_edges)
        )
        self.assertTrue(
            all("骑行落客接驳" in f"{edge.get('name', '')}{edge.get('description', '')}" for edge in bike_dismount_edges)
        )
    ...
    def test_m31a_hust_transport_calibration_walk_bike_and_mixed(self):
        router = Router(GraphLoader.load_site_graph("HUST"))
        ...
        self.assertEqual(south_to_sports_bike["path_steps"][-1]["edge_type"], "bike_lane")
        self.assertIn("南门非机动车绕行接驳", south_to_sports_bike["path_steps"][0]["edge_name"])
        self.assertIn("中心操场非机动车接驳", south_to_sports_bike["path_steps"][-1]["edge_name"])
        mixed_modes = [step["transport_mode_used"] for step in south_to_sports_mixed["path_steps"]]
        self.assertIn("walk", mixed_modes)
        self.assertIn("bike", mixed_modes)
        self.assertEqual(mixed_modes[0], "walk")
        self.assertEqual(south_to_sports_mixed["path_steps"][0]["allowed_transports"], ["walk"])
        self.assertIn("华中科技大学主校区南门步行短接", south_to_sports_mixed["path_steps"][0]["description"])
        self.assertTrue(
            any("华中科技大学主校区步骑共享主路示范段" in step["description"] for step in south_to_sports_mixed["path_steps"])
        )

    def test_m31a_hust_transport_keeps_indoor_segments_walk_only(self):
        router = Router(GraphLoader.load_site_graph("HUST"))
    ...
    def test_m31a_hust_outdoor_data_declares_transport_semantics(self):
        outdoor_path = Path(__file__).resolve().parents[1] / "data" / "sites" / "HUST" / "outdoor.json"
        with outdoor_path.open("r", encoding="utf-8") as f:
            outdoor = json.load(f)
        ...
        shared_edges = [
            edge for edge in edges
            if edge.get("source") == "m31a_hust_transport_calibration"
            and edge.get("transport_semantics") == "shared_walk_bike"
        ]
        bike_calibration_edges = [
            edge for edge in edges
            if edge.get("allowed_transports") == ["bike"]
            and edge.get("source") == "m31a_hust_transport_calibration"
        ]
        bike_gate_detour_edges = [
            edge for edge in bike_calibration_edges
            if edge.get("transport_semantics") == "bike_only"
            and edge.get("m31a_demo_role") == "hust_south_gate_bike_detour"
        ]
        bike_poi_connector_edges = [
            edge for edge in bike_calibration_edges
            if edge.get("transport_semantics") == "bike_dismount_connector"
            and edge.get("m31a_demo_role") == "hust_sports_ground_bike_connector"
        ]
        pedestrian_gate_edges = [
            edge for edge in edges
            if edge.get("source") == "m31a_hust_transport_calibration"
        ...
        self.assertEqual(outdoor["metadata"]["transport_calibration_stage"], "M31A_HUST")
        self.assertEqual(outdoor["metadata"]["transport_modes"], ["walk", "bike", "mixed"])
        self.assertEqual(len(shared_edges), 8)
        ...
        self.assertTrue(
            all("华中科技大学主校区步骑共享主路示范段" in edge.get("description", "") for edge in shared_edges)
        )
        self.assertTrue(all(edge.get("type") == "bike_lane" for edge in bike_gate_detour_edges))
        self.assertTrue(
            all(edge.get("vehicle_access") == "vehicle_only" for edge in bike_gate_detour_edges)
        ...
        self.assertTrue(
            all("华中科技大学" in f"{edge.get('name', '')}{edge.get('description', '')}" for edge in bike_calibration_edges)
        )
        self.assertTrue(
            all("非机动车绕行接驳" in f"{edge.get('name', '')}{edge.get('description', '')}" for edge in bike_gate_detour_edges)
        )
        self.assertTrue(
            all("中心操场非机动车接驳" in f"{edge.get('name', '')}{edge.get('description', '')}" for edge in bike_poi_connector_edges)
        )
    ...
    def test_m31a_scut_transport_calibration_walk_bike_and_mixed(self):
        router = Router(GraphLoader.load_site_graph("SCUT"))
        ...
        self.assertEqual(mixed_modes[0], "walk")
        self.assertEqual(south_to_sports_mixed["path_steps"][0]["allowed_transports"], ["walk"])
        self.assertIn("华南理工大学五山校区南门步行短接", south_to_sports_mixed["path_steps"][0]["description"])
        self.assertTrue(
            any(
    ...
    def test_m31a_scut_transport_keeps_indoor_segments_walk_only(self):
        router = Router(GraphLoader.load_site_graph("SCUT"))
    ...
    def test_m31a_scut_outdoor_data_declares_transport_semantics(self):
        outdoor_path = Path(__file__).resolve().parents[1] / "data" / "sites" / "SCUT" / "outdoor.json"
        with outdoor_path.open("r", encoding="utf-8") as f:
            outdoor = json.load(f)
        ...
        shared_edges = [
            edge for edge in edges
            if edge.get("source") == "m31a_scut_transport_calibration"
            and edge.get("transport_semantics") == "shared_walk_bike"
        ]
        bike_calibration_edges = [
            edge for edge in edges
            if edge.get("allowed_transports") == ["bike"]
            and edge.get("source") == "m31a_scut_transport_calibration"
        ]
        bike_gate_detour_edges = [
            edge for edge in bike_calibration_edges
            if edge.get("transport_semantics") == "bike_only"
            and edge.get("m31a_demo_role") == "scut_south_gate_bike_detour"
        ]
        bike_poi_connector_edges = [
            edge for edge in bike_calibration_edges
            if edge.get("transport_semantics") == "bike_dismount_connector"
            and edge.get("m31a_demo_role") == "scut_sports_ground_bike_connector"
        ]
        pedestrian_gate_edges = [
            edge for edge in edges
            if edge.get("source") == "m31a_scut_transport_calibration"
        ...
        self.assertEqual(outdoor["metadata"]["transport_calibration_stage"], "M31A_SCUT")
        self.assertEqual(outdoor["metadata"]["transport_modes"], ["walk", "bike", "mixed"])
        self.assertEqual(len(shared_edges), 8)
        self.assertEqual(len(bike_calibration_edges), 4)
        self.assertEqual(len(bike_gate_detour_edges), 2)
        ...
        self.assertTrue(
            all("华南理工大学五山校区步骑共享主路示范段" in edge.get("description", "") for edge in shared_edges)
        )
        self.assertTrue(all(edge.get("type") == "bike_lane" for edge in bike_gate_detour_edges))
        self.assertTrue(
            all(edge.get("vehicle_access") == "vehicle_only" for edge in bike_gate_detour_edges)
        ...
        self.assertTrue(
            all("华南理工大学" in f"{edge.get('name', '')}{edge.get('description', '')}" for edge in bike_calibration_edges)
        )
        self.assertTrue(
            all("南门非机动车绕行接驳" in f"{edge.get('name', '')}{edge.get('description', '')}" for edge in bike_gate_detour_edges)
        )
    ...
    def test_m31a_ouc_transport_calibration_walk_bike_and_mixed(self):
        router = Router(GraphLoader.load_site_graph("OUC"))

        south_to_gym_walk = router.query_routing(
            "gate_south",
            "gymnasium",
            strategy="shortest_time",
            transport_mode="walk",
        ...
        south_to_gym_bike = router.query_routing(
            "gate_south",
            "gymnasium",
            strategy="shortest_time",
            transport_mode="bike",
        ...
        south_to_gym_mixed = router.query_routing(
            "gate_south",
            "gymnasium",
            strategy="shortest_time",
            transport_mode="mixed",
        ...
        self.assertTrue(south_to_gym_walk["success"])
        self.assertTrue(south_to_gym_bike["success"])
        self.assertTrue(south_to_gym_mixed["success"])
        self.assertLess(south_to_gym_bike["total_weight"], south_to_gym_walk["total_weight"])
        self.assertLess(south_to_gym_mixed["total_weight"], south_to_gym_bike["total_weight"])
        self.assertEqual(south_to_gym_bike["path_steps"][0]["edge_type"], "bike_lane")
        self.assertIn("南门非机动车绕行接驳", south_to_gym_bike["path_steps"][0]["edge_name"])
        self.assertEqual(
            {step["transport_mode_used"] for step in south_to_gym_bike["path_steps"]},
            {"bike"},
        )
        mixed_modes = [step["transport_mode_used"] for step in south_to_gym_mixed["path_steps"]]
        self.assertIn("walk", mixed_modes)
        self.assertIn("bike", mixed_modes)
        self.assertEqual(mixed_modes[0], "walk")
        self.assertEqual(south_to_gym_mixed["path_steps"][0]["allowed_transports"], ["walk"])
        self.assertIn("中国海洋大学崂山校区南门步行短接", south_to_gym_mixed["path_steps"][0]["description"])
        self.assertTrue(
            any("中国海洋大学崂山校区主路步骑共享" in step["description"] for step in south_to_gym_mixed["path_steps"])
        )

    def test_m31a_ouc_transport_keeps_indoor_segments_walk_only(self):
        router = Router(GraphLoader.load_site_graph("OUC"))
    ...
    def test_m31a_ouc_outdoor_data_declares_transport_semantics(self):
        outdoor_path = Path(__file__).resolve().parents[1] / "data" / "sites" / "OUC" / "outdoor.json"
        with outdoor_path.open("r", encoding="utf-8") as f:
            outdoor = json.load(f)

        edges = outdoor["edges"]
        calibration_edges = [
            edge for edge in edges if edge.get("source") == "m31a_ouc_transport_calibration"
        ]
        shared_edges = [
        ...
        self.assertEqual(outdoor["metadata"]["transport_calibration_stage"], "M31A_OUC")
        self.assertEqual(outdoor["metadata"]["transport_modes"], ["walk", "bike", "mixed"])
        self.assertEqual(len(shared_edges), 10)
        self.assertEqual(len(bike_only_edges), 4)
        ...
        self.assertTrue(all(edge.get("type") == "bike_lane" for edge in bike_only_edges))
        self.assertTrue(
            all(set(edge.get("allowed_transports", [])) == {"walk", "bike"} for edge in shared_edges + bike_connector_edges)
        )
        self.assertTrue(
            any(
                edge.get("from") == "road_central_axis" and edge.get("to") == "road_library_axis"
    ...
    def test_m31a_ynu_transport_calibration_walk_bike_and_mixed(self):
        router = Router(GraphLoader.load_site_graph("YNU"))
        ...
        self.assertLess(south_to_gym_mixed["total_weight"], south_to_gym_bike["total_weight"])
        self.assertEqual(
            {step["transport_mode_used"] for step in south_to_gym_walk["path_steps"]},
            {"walk"},
        )
        self.assertEqual(
            {step["transport_mode_used"] for step in south_to_gym_bike["path_steps"]},
        ...
        self.assertEqual(south_to_gym_bike["path_steps"][0]["edge_type"], "bike_lane")
        self.assertEqual(south_to_gym_bike["path_steps"][-1]["edge_type"], "bike_lane")
        self.assertIn("南门非机动车绕行接驳", south_to_gym_bike["path_steps"][0]["edge_name"])
        self.assertIn("体育馆骑行落客接驳", south_to_gym_bike["path_steps"][-1]["description"])
        mixed_modes = [step["transport_mode_used"] for step in south_to_gym_mixed["path_steps"]]
        self.assertIn("walk", mixed_modes)
        self.assertIn("bike", mixed_modes)
        self.assertEqual(mixed_modes[0], "walk")
        self.assertEqual(south_to_gym_mixed["path_steps"][0]["allowed_transports"], ["walk"])
        self.assertIn("云南大学呈贡校区南门步行短接", south_to_gym_mixed["path_steps"][0]["description"])
        self.assertTrue(
            any(
                "云南大学呈贡校区南区步骑共享主路示范段" in step["description"]
                for step in south_to_gym_mixed["path_steps"]
            )
        )

    def test_m31a_ynu_transport_keeps_indoor_segments_walk_only(self):
        router = Router(GraphLoader.load_site_graph("YNU"))
    ...
    def test_m31a_ynu_outdoor_data_declares_transport_semantics(self):
        outdoor_path = Path(__file__).resolve().parents[1] / "data" / "sites" / "YNU" / "outdoor.json"
        with outdoor_path.open("r", encoding="utf-8") as f:
            outdoor = json.load(f)

        edges = outdoor["edges"]
        calibration_edges = [
            edge for edge in edges if edge.get("source") == "m31a_ynu_transport_calibration"
        ]
        shared_edges = [
            edge for edge in calibration_edges
            if edge.get("transport_semantics") == "shared_walk_bike"
        ...
        self.assertEqual(outdoor["metadata"]["transport_calibration_stage"], "M31A_YNU")
        self.assertEqual(outdoor["metadata"]["transport_modes"], ["walk", "bike", "mixed"])
        self.assertEqual(len(calibration_edges), 14)
        self.assertEqual(len(shared_edges), 8)
        self.assertEqual(len(bike_only_edges), 2)
        self.assertEqual(len(bike_connector_edges), 2)
        self.assertEqual(len(pedestrian_gate_edges), 2)
        ...
        self.assertTrue(
            all("云南大学呈贡校区南区步骑共享主路示范段" in edge.get("description", "") for edge in shared_edges)
        )
        self.assertTrue(all(edge.get("allowed_transports") == ["bike"] for edge in bike_only_edges))
        self.assertTrue(all(edge.get("type") == "bike_lane" for edge in bike_only_edges))
        self.assertTrue(all(edge.get("vehicle_access") == "vehicle_only" for edge in bike_only_edges))
        ...
        self.assertTrue(all(edge.get("vehicle_access") == "vehicle_only" for edge in bike_connector_edges))
        self.assertTrue(
            all("体育馆骑行落客接驳" in f"{edge.get('name', '')}{edge.get('description', '')}" for edge in bike_connector_edges)
        )
        self.assertTrue(
            any(
                edge.get("from") == "road_south_gate" and edge.get("to") == "road_sports_axis"
                for edge in shared_edges
            )
        )
        self.assertTrue(
    ...
    def test_m31a_hzau_transport_calibration_walk_bike_and_mixed(self):
        router = Router(GraphLoader.load_site_graph("HZAU"))
        ...
        self.assertEqual(mixed_modes[0], "walk")
        self.assertEqual(south_to_gym_mixed["path_steps"][0]["allowed_transports"], ["walk"])
        self.assertIn("华中农业大学狮子山校区南门步行短接", south_to_gym_mixed["path_steps"][0]["description"])
        self.assertTrue(
            any(
    ...
    def test_m31a_hzau_transport_keeps_indoor_segments_walk_only(self):
        router = Router(GraphLoader.load_site_graph("HZAU"))
    ...
    def test_m31a_hzau_outdoor_data_declares_transport_semantics(self):
        outdoor_path = Path(__file__).resolve().parents[1] / "data" / "sites" / "HZAU" / "outdoor.json"
        with outdoor_path.open("r", encoding="utf-8") as f:
            outdoor = json.load(f)

        edges = outdoor["edges"]
        calibration_source = "m31a_hzau_transport_calibration"
        calibration_edges = [
            edge for edge in edges if edge.get("source") == calibration_source
        ]
        shared_edges = [
            edge for edge in calibration_edges
            if edge.get("transport_semantics") == "shared_walk_bike"
        ...
        self.assertEqual(outdoor["metadata"]["transport_calibration_stage"], "M31A_HZAU")
        self.assertEqual(outdoor["metadata"]["transport_modes"], ["walk", "bike", "mixed"])
        self.assertEqual(len(calibration_edges), 14)
        self.assertEqual(len(shared_edges), 8)
        self.assertEqual(len(bike_only_edges), 2)
        self.assertEqual(len(bike_connector_edges), 2)
        self.assertEqual(len(pedestrian_gate_edges), 2)
        self.assertTrue(all(edge.get("source") == calibration_source for edge in calibration_edges))
        self.assertTrue(all(edge.get("allowed_transports") == ["walk"] for edge in pedestrian_gate_edges))
        self.assertTrue(all(edge.get("vehicle_access") == "pedestrian_only" for edge in pedestrian_gate_edges))
        self.assertTrue(
            all(set(edge.get("transport_speeds", {}).keys()) == {"walk"} for edge in pedestrian_gate_edges)
        ...
        self.assertTrue(
            all("华中农业大学狮子山校区步骑共享主路示范段" in edge.get("description", "") for edge in shared_edges)
        )
        self.assertTrue(all(edge.get("allowed_transports") == ["bike"] for edge in bike_only_edges))
        self.assertTrue(all(edge.get("type") == "bike_lane" for edge in bike_only_edges))
        self.assertTrue(all(edge.get("vehicle_access") == "vehicle_only" for edge in bike_only_edges))
